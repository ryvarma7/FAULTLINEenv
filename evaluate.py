...
from inference import TASKS, run_task, API_BASE_URL, API_KEY
...
def main():
    ...
    print("\n" + "="*40)
    print(f"{'Task':<25} | {'Avg Score':<10} | {'Success Rate':<12}")
    print("-" * 53)
    for task in TASKS:
        summary = results[task + "_summary"]
        print(f"{task:<25} | {summary['avg_score']:<10.3f} | {summary['success_rate']:<12.1%}")
    print("-" * 53)
