...
class TestFaultLineEnvReset:
    ...
    def test_reset_is_reproducible(self):
        ...
        r1 = env1.reset()
        r2 = env2.reset()
        assert r1.observation.alerts[0].id == r2.observation.alerts[0].id
        assert r1.observation.alerts[0].description == r2.observation.alerts[0].description

    def test_dependency_graph_present(self):
        env = FaultLineEnv("single_service_latency", seed=42)
        result = env.reset()
        assert "api-gateway" in result.observation.dependency_graph
        assert "elasticsearch" in result.observation.dependency_graph
