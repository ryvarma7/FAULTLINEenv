...
class TaskHard(BaseTask):
    ...
    def __init__(self, seed: int = 42):
        super().__init__(seed)
        with open(DATA_DIR / "incidents" / "hard.json") as f:
            self.config = json.load(f)
        self.red_herring_services = ["elasticsearch", "search-service"]
        self._red_herring_penalties_applied: set = set()
        self._queried_fraud_detector_logs = False
        self._queried_model_serving_logs = False
    ...
    def step(self, action: FaultLineAction) -> StepResult:
        ...
        if isinstance(action, AcknowledgeAlertAction):
            ...
            if self.log_query_counts[key] > 3:
                ...
                # Red herring penalty - apply once per red herring service
                if action.service in self.red_herring_services and action.service not in self._red_herring_penalties_applied:
                    step_reward -= 0.05
                    self.add_reward(f"red_herring_penalty_{action.service}", -0.05)
                    self._red_herring_penalties_applied.add(action.service)
                
                # Reward for querying specific services
                if action.service == "fraud-detector" and not self._queried_fraud_detector_logs:
                    step_reward += 0.05
                    self.add_reward("queried_fraud_detector_logs", 0.05)
                    self._queried_fraud_detector_logs = True
                elif action.service == "model-serving" and not self._queried_model_serving_logs:
                    step_reward += 0.05
                    self.add_reward("queried_model_serving_logs", 0.05)
                    self._queried_model_serving_logs = True
                
                # Determine anomaly type based on service
                if action.service == "model-serving":
                    anomaly_type = "CPU_THROTTLE"
                elif action.service == "fraud-detector":
            ...
            obs = self._build_obs(result_message, log_results=generate_logs(
                action.service,
                "CPU_THROTTLE" if action.service == "model-serving" else
                "TIMEOUT" if action.service == "fraud-detector" else "NORMAL",
                self.seed
            ))
        ...
        elif isinstance(action, ScaleServiceAction):
            done = True
            if action.service == self.get_root_cause_service():
                step_reward += 0.35
                self.add_reward("correct_scale_service", 0.35)
                if action.replicas == 4:
                    step_reward += 0.05
                    self.add_reward("correct_replica_count", 0.05)
                self.episode_state = EpisodeState.RESOLVED
                if self.elapsed_steps <= 14:
                    step_reward += 0.05
                    self.add_reward("speed_bonus", 0.05)
            else:
                step_reward -= 0.20
                self.add_reward("wrong_scale_target", -0.20)
                self.episode_state = EpisodeState.FAILED
            result_message = f"Scale action submitted for service: {action.service} to {action.replicas} replicas."
